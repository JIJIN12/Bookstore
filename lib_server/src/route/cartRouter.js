...
cartRouter.get('/:id',CheckAuth,async function(req,res){
    try {
        ...
        if (favourite_save) {
            return res.status(200).json({ success: "true", error: "false", Message: "Book added", details: cart_save })
...
cartRouter.get('/useritems/:id',async function(req,res){
    try {
        ...
        if(cart_item[0]){
            return res.status(200).json({ success: "true", error: "false",  details: cart_item })

        }
        else{
            return res.status(400).json({ success: "false", error: "true", Message: "cart item data not found" })
